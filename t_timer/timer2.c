...
// FONTS
TTF_Font* g_font = 0;
texture 	g_start_prompt_texture = { .is_allocated = 0, 0 };
texture 	g_pause_prompt_texture = { .is_allocated = 0, 0 };
		...
		while	 ( !user_quit ) {
			...
			while ( SDL_PollEvent(&g_event) != 0 ) {
				...
				else if( g_event.type == SDL_KEYDOWN )
				{
					...
					// pause / unpause timer
					else if ( g_event.key.keysym.sym == SDLK_p )
					{
						if ( timer_is_paused(&g_timer) )
						{
			...
			strcpy(g_timer_text, "");
			sprintf(g_timer_text, "seconds %f", timer_get_ticks(&g_timer) / 1000.f);
