...
int game_update() {
  ...
  if ( SDL_RenderSetViewport(g_render, &mini) < 0) {
		printf("viewport err: %s\n", SDL_GetError());
	}

	// Render texture to screen
	SDL_RenderCopy(g_render, g_texture, 0, 0);
	
	//Bottom viewport
  SDL_Rect mini_bottom;
  mini_bottom.x = 0;
  mini_bottom.y = SCREEN_HEIGHT / 2;
  mini_bottom.w = SCREEN_WIDTH;
  mini_bottom.h = SCREEN_HEIGHT / 2;
  SDL_RenderSetViewport(g_render, &mini_bottom);

	if ( SDL_RenderSetViewport(g_render, &mini_bottom) < 0) {
		printf("viewport err: %s\n", SDL_GetError());
	}
